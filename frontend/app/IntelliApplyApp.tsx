...
export default function IntelliApplyApp({ initialHref = "/" }: { initialHref?: string }) {
  // `page`/`setPage` keep the exact shape the screens below already expect, but
  // are now backed by the URL so deep links and browser back/forward work.
  const router = useAppRouter(initialHref);
  const { page, setPage } = router;
  ...
  // Application Hub state — the selected application now lives in the URL as
  // `/applications/:appId`, so a detail view can be linked to and restored.
  const selectedAppId = router.appId;
  const setSelectedAppId = router.setAppId;
  ...
  const handleEnterDemo = useCallback(() => {
    enterDemo();
    // Go to the demo dashboard and record demo mode in the URL (`?demo=1`).
    router.go({ page: "landing", demo: true });
  }, [enterDemo, router]);

  const handleExitDemo = useCallback(() => {
    exitDemo();
    router.go({ page: "landing", demo: false, appId: null, jobId: null });
  }, [exitDemo, router]);

  // Select a demo job and remember it in the URL so returning to the analysis
  // screen via back/forward (or a shared link) resolves the same analysis.
  const selectDemoJob = useCallback((jobId: string) => {
    selectJob(jobId);
    router.setJobId(jobId);
  }, [selectJob, router]);

  // `?demo=1` is the source of truth for demo mode, so it survives a reload and
  // a shared link opens in demo. Reconciling in both directions also means
  // stepping back/forward across the point where demo was entered or exited
  // restores the right mode instead of stranding the UI in the other one.
  useEffect(() => {
    if (router.route.demo && !isDemo) enterDemo();
    else if (!router.route.demo && isDemo) exitDemo();
  }, [router.route.demo, isDemo, enterDemo, exitDemo]);

  // Back/forward between demo screens changes `?job=` in the URL; keep the demo
  // hook's selection in step with it.
  useEffect(() => {
    if (!isDemo) return;
    const jobId = router.route.jobId;
    if (jobId && jobId !== selectedJobId) selectJob(jobId);
  }, [isDemo, router.route.jobId, selectedJobId, selectJob]);
  ...
  return <main className={shell ? "app-shell" : "landing-shell"}>
    ...
    <section className={shell ? "app-main" : "landing-main"}>
      ...
      <div className={shell ? "page-wrap" : ""}>
        ...
        {page === "landing" && isDemo && <DemoDashboard data={demo.data} setPage={setPage} selectJob={selectDemoJob} setSelectedAppId={setSelectedAppId} />}
        ...
        {page === "jobs" && isDemo && <DemoJobsPage data={demo.data} selectJob={selectDemoJob} setPage={setPage} />}
        ...
        {page === "analysis" && isDemo && <DemoAnalysisPage analysis={selectedAnalysis} data={demo.data} setPage={setPage} selectJob={selectDemoJob} />}
        ...
        {page === "history" && isDemo && <DemoHistoryPage data={demo.data} selectJob={selectDemoJob} setPage={setPage} setSelectedAppId={setSelectedAppId} />}
        ...
        {page === "application-detail" && isDemo && <DemoApplicationDetailPage data={demo.data} appId={selectedAppId} setPage={setPage} setSelectedAppId={setSelectedAppId} selectJob={selectDemoJob} />}
